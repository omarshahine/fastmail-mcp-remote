 ...
 */
export function isToolAllowed(userConfig: UserConfig, toolName: string, args?: Record<string, unknown>): PermissionResult {
 ...
 */
export async function checkMcpPermissions(request: Request, userLogin: string, kv: KVNamespace): Promise<Response | null> {
  ...
  for (const msg of messages) {
    if (typeof msg !== "object" || msg === null || (msg as Record<string, unknown>).method !== "tools/call") {
      continue;
    }
 ...
 */
export async function filterToolsListResponse(response: Response, userLogin: string, kv: KVNamespace): Promise<Response> {
  ...
  if (typeof body !== "object" || body === null || !("result" in (body as Record<string, unknown>))) {
    return response;
  }
  ...
  // Filter tools
  const filteredTools = (result.tools as Array<Record<string, unknown>>).filter((tool) => {
  ...
  // Create new headers without Content-Length (will be recalculated for new body)
