...
function isLayoutTable(table: DomNode): boolean {
  ...
  // Only treat cellpadding="0" cellspacing="0" as layout table if there are no headers
  if (table.getAttribute("cellpadding") === "0" && table.getAttribute("cellspacing") === "0" && table.querySelectorAll("th").length === 0)
    return true;
  if (table.querySelectorAll("th").length === 0) return true;
  return false;
}
...
const BOILERPLATE_SELECTORS = ['[class*="signature"]', '[id*="signature"]', ".gmail_extra", ".gmail_signature", ".yahoo_quoted"].join(", ");

/** Tags that are considered empty if they have no text and no media children. */
const EMPTY_ELEMENT_TAGS = new Set(["SPAN", "DIV", "P", "A", "STRONG", "EM", "FONT", "B", "I", "U"]);
...
function removeEmptyElements(document: any): void {
  ...
  while (removed) {
    removed = false;
    for (const el of document.querySelectorAll(Array.from(EMPTY_ELEMENT_TAGS).join(", ").toLowerCase())) {
      ...
      const hasMeaningful = Array.from(el.querySelectorAll("*") as ArrayLike<any>).some((child: any) =>
        MEANINGFUL_CHILDREN.has(child.nodeName),
      );
