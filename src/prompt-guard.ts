...
function markItem(item: Record<string, unknown>, domain: string): Record<string, unknown> {
	...
	// Handle nested email address objects (from, to, cc, bcc, replyTo)
	if (domain === "mail") {
		for (const addrField of ["from", "to", "cc", "bcc", "replyTo"]) {
...
function markToolResult(result: unknown, toolName: string): unknown {
	...
	// Calendar results
	if (toolName.startsWith("calendar_") || toolName === "list_calendar_events" || toolName === "get_calendar_event" || toolName === "list_calendars" || toolName === "create_calendar_event") {
	...
	if (toolName.startsWith("mail_") || toolName.includes("email") || toolName === "list_emails" || toolName === "get_email" || toolName === "search_emails" || toolName === "get_recent_emails" || toolName === "advanced_search" || toolName === "get_thread") {
