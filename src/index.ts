...
async function handleSse(c: {
  req: { raw: Request; url: string; header: (name: string) => string | undefined };
  env: Env;
  ...
  // Pass through to MCP SDK
  const response = await FastmailMCP.serveSSE("/sse").fetch(c.req.raw, c.env, c.executionCtx);
