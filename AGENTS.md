...
│   └── tools/
│       ├── email.ts                 # 28 email tools (read + write + organize + bulk)
...
The `openclaw-plugin/` directory is published to npm as `fastmail-cli`. It registers 38 OpenClaw agent tools that shell out to the `fastmail` CLI. Zero runtime dependencies — the CLI handles MCP connection, auth, and formatting.
